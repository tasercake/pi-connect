# Issue #5 Fix Plan: Pi Context Safety for Long cc-connect Sessions

## Scope
Implement the fix in `tasercake/cc-connect` only. The concrete failure path is cc-connect's Pi adapter and engine integration: Pi sessions are resumed indefinitely, Pi does not satisfy `core.ContextCompressor`, Pi usage telemetry is not surfaced as cumulative context load, and the adapter emits terminal errors before Pi's own overflow compaction/retry can recover.
...
2. Pi usage events include `cacheRead`/`totalTokens`, but cc-connect only sees `input/output` or zero, so logs/context indicators/auto-compress miss cumulative context load.
3. Engine auto-compress uses cc-connect's text-history estimate; this misses tool-heavy Pi sessions where provider context is dominated by cached/tool messages.
...
- Parse usage from all Pi event shapes cc-connect may receive: JSON `message_end.message.usage`, JSON `message.usage` records, RPC `turn_end.message.usage`, and RPC `agent_end.messages[].usage`.
...
Files: `cmd/cc-connect/main.go`, `config.example.toml`, config tests if needed
...
Tests: add/adjust focused command/config wiring tests only if an existing test harness covers this path cheaply; otherwise verify with `go test ./cmd/cc-connect`.
...
- Store it as pending instead of immediately emitting `EventError`; this is required because cc-connect cannot send `/compact` while a Pi JSON/RPC turn is still running, and the observed failure happened mid-turn before Pi's own overflow compaction/retry.
...
Run:
```bash
gofmt -w agent/pi/pi.go agent/pi/session.go agent/pi/session_rpc.go agent/pi/pi_test.go core/engine.go core/engine_test.go cmd/cc-connect/main.go
go test ./agent/pi ./core ./cmd/cc-connect
go test ./...
```
