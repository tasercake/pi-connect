...
本文档介绍如何将 **pi-connect** 接入飞书，让你可以通过飞书机器人远程调用 Pi。
...
如果你已经装好 `pi-connect`，可以直接用内置命令完成“新建机器人/关联已有机器人”，并自动写回 `config.toml`：
...
> ⚠️ **重要**：请妥善保存这两个凭证，后续配置 pi-connect 时需要用到。App Secret 只会显示一次，如果忘记了需要重置。
...
启动后，pi-connect 会自动与飞书建立 WebSocket 长连接。你会在日志中看到：
...
pi-connect: ✅ 这是一个 Node.js 项目，包含以下目录...
```
...
pi-connect 内置了自动重连机制，断开后会自动尝试重新连接。
...
pi-connect 默认使用交互卡片显示权限确认、provider 选择等操作。如果点击按钮后无响应、显示加载超时或报错，请检查：
...
通常是飞书注册模板侧的展示文案，不影响返回 `app_id/app_secret` 和接入 pi-connect。
