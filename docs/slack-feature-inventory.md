...
- **Session keys**: Explicit typed formats (`slack:u`, `slack:c`, `slack:ut`, `slack:t`)
...
- `session_per_thread` config option (default `false`)
- Session keys use only these formats: `slack:u:<channel>:<user>`, `slack:c:<channel>`, `slack:ut:<channel>:<user>:<thread_ts>`, `slack:t:<channel>:<thread_ts>`
...
- Bridges direct CLI usage and pi-connect sessions
...
| `session_per_thread`       | No       | Scope sessions per Slack thread (`thread_ts` when present, otherwise message `ts`) |
