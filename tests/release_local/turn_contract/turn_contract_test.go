...
func TestReplyMetadataConfigurationMatrix(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			showCtx:    true,
			showFooter: true,
			want:       []string{"answer", "*[ctx: ~14%] · glm-5.1 · worked ", " · …/tmp/release-agent*"},
		},
		{
			name:       "context_off_footer_on_keeps_model_footer",
			showCtx:    false,
			showFooter: true,
			want:       []string{"answer", "*glm-5.1", " · worked ", " · …/tmp/release-agent*"},
			forbid:     []string{"[ctx:"},
		},
		{
		...
		},
		{
			...
			showFooter: false,
			want:       []string{"answer"},
			forbid:     []string{"[ctx:", "glm-5.1", "worked ", "/tmp/release-agent"},
		},
	}
