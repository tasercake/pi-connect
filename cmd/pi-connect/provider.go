...
func printProviderUsage() {
	fmt.Println(`Usage: pi-connect provider <command> [options]
...
Examples:
  pi-connect provider add --project my-backend --name relay --api-key sk-xxx
  pi-connect provider add --project my-backend --name bedrock --env CLAUDE_CODE_USE_BEDROCK=1,AWS_PROFILE=bedrock
  pi-connect provider list --project my-backend
  pi-connect provider remove --project my-backend --name relay
  pi-connect provider presets
  pi-connect provider global list
  pi-connect provider global add --name minimaxi --api-key sk-xxx --base-url https://api.minimaxi.chat/v1`)
}
...
func runProviderGlobal(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, `Usage: pi-connect provider global <command>
...
func runGlobalProviderList(args []string) {
	...
	if len(providers) == 0 {
		...
		fmt.Println("\nAdd one with: pi-connect provider global add --name <name> --api-key <key>")
		return
	}
