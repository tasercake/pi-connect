...
func runCronAdd(args []string) {
	...
	// Fallback to env vars (set by pi-connect when spawning agent)
	if project == "" {
	...
	if _, err := os.Stat(sockPath); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "Error: pi-connect is not running (socket not found: %s)\n", sockPath)
		os.Exit(1)
	}
...
func runCronInfo(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Usage: pi-connect cron info <id> [field]")
		fmt.Fprintln(os.Stderr, "Use 'pi-connect cron list' to see all task IDs.")
		os.Exit(1)
	}
...
func runCronEdit(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Run 'pi-connect cron edit --help' for usage.")
		os.Exit(1)
	}
	if field == "" {
...
func printCronUsage() {
	fmt.Println(`Usage: pi-connect cron <command> [options]
...
Run 'pi-connect cron <command> --help' for details.`)
}

func printCronAddUsage() {
	fmt.Println(`Usage: pi-connect cron add [options] [<min> <hour> <day> <month> <weekday> <prompt>]
...
Options:
  ...
  -c, --cron <expr>          Cron expression, e.g. "0 6 * * *"
      ...
      --data-dir <path>      Data directory (default: ~/.pi-connect)
  -h, --help                 Show this help

Examples:
  pi-connect cron add --cron "0 6 * * *" --prompt "Collect GitHub trending data" --desc "Daily Trending"
  pi-connect cron add --cron "*/30 * * * *" --exec "df -h" --desc "Disk usage check"
  pi-connect cron add 0 6 * * * Collect GitHub trending data and send me a summary`)
}

func printCronEditUsage() {
	fmt.Println(`Usage: pi-connect cron edit <id> <field> <value> [options]
...
Examples:
  ...
  pi-connect cron edit abc123 description "Daily standup reminder"
  pi-connect cron edit abc123 timeout_mins 60
  pi-connect cron edit abc123 mute true`)
}
