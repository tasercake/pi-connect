...
func resolveSocketPath(dataDir string) string {
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".pi-connect", "run", "api.sock")
	}
	return filepath.Join(".pi-connect", "run", "api.sock")
}

func printSendUsage() {
	fmt.Println(`Usage: pi-connect send [options] <message>
       pi-connect send [options] -m <message>
       pi-connect send [options] --stdin < file
       pi-connect send [options] --image <path>
       pi-connect send [options] --file <path>
       echo "msg" | pi-connect send [options] --stdin

Send a message or attachment to an active pi-connect session.
...
Examples:
  pi-connect send "Daily summary: ..."
  pi-connect send -m "Build completed successfully"
  pi-connect send --message "Chart generated" --image /tmp/chart.png
  pi-connect send --file /tmp/report.pdf
  pi-connect send --stdin <<'EOF'
