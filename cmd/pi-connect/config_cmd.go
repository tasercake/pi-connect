...
func printConfigUsage() {
	fmt.Fprintf(os.Stderr, `Usage: pi-connect config <subcommand>
...
Examples:
  pi-connect config example              Print example config
  pi-connect config example > config.toml  Save example config
  pi-connect config format               Format default config file
  pi-connect config fmt --config /path/to/config.toml
`)
}
