...
func runSessions(args []string) {
	...
	case "show":
		...
		if id == "" {
			...
			fmt.Fprintln(os.Stderr, "Usage: pi-connect sessions show <session-id> [-n N]")
			os.Exit(1)
		}
...
func resolveDataDir(flagValue string) string {
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".pi-connect")
	}
	return ".pi-connect"
}
...
func runSessionsShow(dataDir, id string, limit int) {
	...
	if record == nil {
		...
		fmt.Fprintln(os.Stderr, "Use 'pi-connect sessions list' to see available sessions.")
		os.Exit(1)
	}
...
func printSessionsUsage() {
	fmt.Println(`Usage: pi-connect sessions [command] [options]
...
Examples:
  pi-connect sessions                           Interactive TUI browser
  pi-connect sessions list                      List all sessions
  pi-connect sessions show "mybot:s1"           Show all messages in session
  pi-connect sessions show "#1" -n 20           Show last 20 messages of first session`)
}
