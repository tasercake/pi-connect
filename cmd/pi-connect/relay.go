...
func printRelayUsage() {
	fmt.Println(`Usage: pi-connect relay <command> [options]

Commands:
  send      Send a message to another bot via relay

Run 'pi-connect relay <command> --help' for details.`)
}

func printRelaySendUsage() {
	fmt.Println(`Usage: pi-connect relay send [options] [<target_project> <message>]
...
Examples:
  pi-connect relay send --to claude-bot "What's the weather today?"
  pi-connect relay send claude-bot What is the weather today`)
}
