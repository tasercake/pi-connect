...
func printAgentSIDUsage() {
	fmt.Println(`Usage: pi-connect agent-sid [options]

Print the Pi session ID for the
current session. This is the ID used for --resume.

The command reads from the persisted session file; no running pi-connect
...
Examples:
  pi-connect agent-sid                         Auto-detect from env (inside a session)
  pi-connect agent-sid -p mybot -s "discord:123:456"`)
}
